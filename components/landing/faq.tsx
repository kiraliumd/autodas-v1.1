    ...
    <section className="py-12 md:py-16 lg:py-20 bg-[#FFF8EE]" id="faq">
      <div className="container mx-auto max-w-[1120px] px-4 sm:px-6">
        <div className="text-center max-w-3xl mx-auto mb-8 md:mb-12 animate-fade-in">
          ...
          <p className="mt-3 md:mt-4 text-base sm:text-lg text-gray-700">Tire suas dúvidas sobre o AutoDAS</p>
        </div>
          ...
          <Accordion type="single" collapsible className="w-full">
            {faqs.map((faq, index) => (
              <AccordionItem key={index} value={`item-${index}`} className={`animate-fade-in delay-${index * 100}`}>
                <AccordionTrigger className="text-left font-medium text-sm md:text-base text-gray-900">
                  {faq.question}
                </AccordionTrigger>
                <AccordionContent className="text-xs md:text-sm text-gray-700">{faq.answer}</AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
        </div>
